...
def train_local(model: nn.Module, train_dataloader, validation_dataloader, num_epochs):
  ...
  optimizer = torch.optim.Adam([{'params': model.get_pretrained_parameters(), 'lr':1e-5},
                              {'params': model.get_random_initialized_parameters()}], lr=1e-4)
  ...
  for e in range(num_epochs):
    ...
    #   if pseudo_label != None:
    #     class_loss = pseudo_labeling_criterion(pseudo_label, resized_labels, weights=None)
    #   else:
    #      class_loss = 0
